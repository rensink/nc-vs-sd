...
\path
  ...
  (Iw) edge[->] node[opengraph] {} node[right] {$d_w$} (Pw)
  (Iw) edge[->] node[right] {$u_w$} (Pv);
  
\node (Ix) [right=2 of Iv] {};
\node (Px) [below=of Ix] {};
...
\path[allcolor=gray]
  (Iy) edge[->] node[opengraph] {} node[left] {$d_y$} (Py)
  (Iv) edge[->] node[right] {$u_x$} (Pz)
  (Iy) edge[->] node[left] {$u_y$} (Pz)
  (Iy) edge[->,bend right=15] node[above,near start] {$k$} (Ix.south west);

\path[morphism=blue!40!white]
  (Iy) edge[->] node[left,pos=.6] {$b^I$} (Iw.north west)
  (Py) edge[->] node[left,pos=.6] {$b^P$} (Pw.north west);
