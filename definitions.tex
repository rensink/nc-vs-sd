...
Given a morphism $\cM$, we define a parent relation ${\childof}\subseteq \cM\times \cM$ as follows:
...
\item \emph{(Child-based reflection evidence.)} $\trip ybw\in\cM$ provides \emph{child-based reflection evidence}, denoted $\isrechd b{x,a}$, if $x\parentof y$ and there is a mediating morphism $k\of I_y\to I_x$ such that $k;d_x=u_y$ and $k;u_x;a\P=d_y;b\P$.
...
\rechd{a,x} ={} & \gensetof{\trip ybw\in \cM}{x\parentof y, \exists k:I_y\to I_x\st k;d_x=u_y\wedge k;u_x;a\P=d_y;b\P} \\
...
Let $\cM\of \cT\to\cU$ be a morphism.
\begin{itemize}[topsep=\smallskipamount]
\item $\trip vaw\in \cM$ \emph{preserves a $\hat v$-model $g$} if $a\I$ is epi and $g=a\I;g'$ implies $g'\sat \hat w$.

\item $\trip vaw\in \cM$ \emph{preserves models} if it preserves all $\hat v$-models. 

\item $\cM$ preserves models if $\rt\cM$ preserves models.
\end{itemize}
\end{definition}
%
Note that for the root triple $a=\rt\cM$, we have $a\I=\id_I$, which is epi, and moreover implies that $g=a\I;g'$ if and only if $g=g'$; so model preservation (of $\cM$) simply means that all $\cT$-models are also $\cU$-models, as expected.
%
...
Let $\cM\of \cT\to\cU$ be a condition tree morphism and let $\trip vaw\in\cM$.
\begin{itemize}[topsep=\smallskipamount]
\item \emph{(Interface shift.)} $a$ is an \emph{interface shift}, denoted $a\in \ishift$, if there is an arrow $a\Q\of P_w\to Q_v$ with $Q_v$ the pushout object of $(u_v,d_v)$.
...
\end{definition}
%
With respect to interface shift: if $(v,a,w)=\rt\cM$, then $u_v=\id_I$ and hence $P_v$ itself is the pushout of $(u_v,d_v)$; hence $a\in\ishift$ if and only if $a\Q$ is a section of $a\P$.
%
For a visualisation see \cref{fig:preservation evidence}.
%
\begin{figure}
	\input{figs/preservation-evidence}
	\caption{The notions of preservation evidence of \cref{def:preservation evidence}}
	\label{fig:preservation evidence}
\end{figure}

\begin{definition}[syntactic preservation]
Let $\cM\of \cT\to\cU$ be an CT-morphism, and let $\trip vaw\in\cM$.
\begin{itemize}[topsep=\smallskipamount]
\item \emph{(Direct preservation.)} $a:\pdir$ if for every child $y$ of $w$, there is some $b:\pedir{y,a}$ such that $b:\pdir$.
\item \emph{(Child-based preservation.)} $a:\pchd$ if one of the following holds:
\begin{itemize}
\item for every child $y$ of $w$, there is some $b:\pedir{y,a}$ such that $b:\pchd$;
