...
\item for all $c=(v,w)\in C$, $u_w$ is an open graph with $I_{c_w}=I_v$ and $P_{c_w}=P_w$. \todo{AC:I read $(v,w)$ as the edge from the parent $v$  to the child $w$. Then $u_w$ is simply a morphism $u_w \of I_w \to P_v$. Or, if seen as an open graph, $I_{u_w}=I_w$ and $P_{u_w}=P_v$}
\end{itemize}
\end{definition}
%
...
Let $\cT$ be a condition tree, $v$ a vertex $v\in V_\cT$,  and $g\of I_v\to G$ an assignment. $g$ \emph{satisfies $v$},\todo{AC: better $g$ satisfies $\hat v$? Then we don't need a special case for $\cT$, as $\cT = \widehat{\rt\cT}$.} denoted $g\sat v$, if there is a graph morphism $h\of P^v\to g$ such that
\begin{inumerate}
\item $g=d_v;h$, and 
...
Given two condition trees $\cT,\cU$ with $I_\cT=I_\cU$, a morphism $\cM$ from $\cT$ to $\cU$ is a set of triples $(v,a,w)$ where $(v,w)\in (V_\cT\times V_\cU)\cup (V_\cU\times V_\cT)$ and $a\of d_v\to d_w$ is an open graph morphism, such that $\rt\cM=(\rt\cT,(\id,r),\rt\cU)\in \cM$ for some $r$.\todo{AC: Formally a morphism should be a set of triples + a distinguished triple $\rt\cM$, which is needed to fix source and target (because in principle there could be more triples between the roots). If so, the distinguished triple should be handled in what follows.}
\end{definition}
%
...
We are interested in condition tree morphisms that either preserve or reflect satisfaction, in the following sense.

\begin{definition}[preservation and reflection of models]
Let $\cM\of \cT\to\cU$ be a morphism and let $\trip vaw\in\cM$.
\begin{itemize}[topsep=\smallskipamount]
\item $a$ \emph{preserves} a $v$-model $g\sat v$ with witness $h$ if $g=a^I;g'$ and $h=a^P;h'$ for some $g',h'$ such that $g'\sat w$ with witness $h'$. $\cM$ preserves models if $\rt\cM$ preserves all $v$-models.\todo{AC: should be $\cT$-models?}
\item $a$ \emph{reflects} a $w$-model $g\sat w$ with witness $h$ if $a^I;g\sat v$ with witness $a^P;h$. $\cM$ reflects models if $\rt\cM$ reflects all $w$-models. \todo{AC: all $\cU$-models?}
\end{itemize}
\end{definition}
...
\begin{lemma}[syntactic reflection implies semantic reflection]\label{lem:reflection}
